...
public class GoogleVoiceSpeech : MonoBehaviour
{
    ...
    //                GetComponent<AudioVisualizer>().VisualizeVoice();
    ...
    //                string apiURL = "http://www.google.com/speech-api/v2/recognize?lang=ko-KR&key=" + apiKey;
    //                // string apiURL = "https://speech.googleapis.com/v1/speech:recognize?lang=ko-KR&key=" + apiKey;
    //                string Response;
    ...
    public IEnumerator StopOperation()
    {
        if (micConnected)
        {
            ...
            if (GUI.Button(new Rect(Screen.width / 2 - 100, Screen.height / 2 - 25, 200, 50), "Stop and Play!"))
            {
                ...
                //StartCoroutine(HttpUploadFile(apiURL, filePath, "file", "audio/wav; rate=44100"));
                Debug.Log(string.Format("Uploading {0} to {1}", filePath, apiURL));
                ...
                int bytesRead = 0;
                //StartCoroutine(ReadStream(fileStream, rs, buffer, bytesRead));

...
public class SavedPagesScroll : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    public bool SnapToggle = false;
    ...
    public int numExtraPagesShown = 1;
    ...
    public delegate void ActivePageChangedDelegate(Transform activePage, int activePageIndex, Transform previousPage, int previousPageIndex);
    ...
    private Dictionary<int, Transform> indexToVisiblePage = new Dictionary<int, Transform>();
    private Dictionary<Transform, int> visiblePageToIndex = new Dictionary<Transform, int>();
    ...
    private List<Transform> visiblePages = new List<Transform>();
    ...
    public Transform ActivePage
    {
        ...
        private set
        {
            ...
            Transform previousPage = ActivePage;
    ...
    /// <param name="immediate">If set to <c>true</c> then snapping happens instantly,
    /// otherwise it is animated.</param>
    public void SnapToVisiblePage(Transform visiblePage, bool immediate = false)
    {
        if (visiblePage == null)
        {
    ...
    public void Reset()
    {
        foreach (KeyValuePair<Transform, int> pair in visiblePageToIndex)
        {
    ...
    void Start()
    {
        pageProvider = GameObject.FindObjectOfType<RedisManager>().GetComponent<IPageProvider>();
    ...
    private void OnScrolled()
    {
        ...
        {
            Transform page = visiblePages[i];

            /// If this object doesn't have a Transform it isn't a valid page.
    ...
    {
        if (TutorialManager.Instance.isFirst) TutorialManager.Instance.DoMission(14);
    ...
    private void AddPage(int index, bool isActivePage = false)
    {
        ...
        Transform page = pageProvider.ProvidePage(pageIndex);
    ...
    public void RemovePage(Transform page)
    {
    ...
    private void ApplyScrollEffects(Transform page)
    {
        int index = visiblePageToIndex[page];
        float offset = OffsetFromIndex(index);

        bool isActivePage = page == activePage;
        bool isInteractable = isActivePage;

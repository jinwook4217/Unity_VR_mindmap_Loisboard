...
public class GvrReticlePointerImpl : GvrBasePointer
{
       ...
       Ray intersectionRay, bool isInteractive)
    {
        ...
        //포인트 엔터일 때
        // ReticleManager.isRotation = true;
        if (ReticleManager.Instance.isInfiniteMode)
        {
            ReticleManager.Instance.isInfiniteRotation = true;

        }
        ReticleManager.Instance.TriggerCoroutine();
    }
    ...
    public override void OnPointerExit(GameObject targetObject)
    {
        ...
        //custom
        ReticleManager.Instance.isInfiniteRotation = false;
    }
    ...
    public void UpdateDiameters(float reticleSizeValue)
    {
        ReticleDistanceInMeters =
          Mathf.Clamp(ReticleDistanceInMeters, RETICLE_DISTANCE_MIN, RETICLE_DISTANCE_MAX);
        ...
        float inner_diameter = 2.0f * reticleSizeValue * Mathf.Tan(inner_half_angle_radians);
        float outer_diameter = 2.0f * reticleSizeValue * Mathf.Tan(outer_half_angle_radians);
        ...
        if (ReticleInnerDiameter > inner_diameter - 0.1)
        {
            ReticleManager.isLarger = false;
        }
    ...
    //custom function
    public void MakeReticleOnce(float reticleSizeValue)
    {
        SetPointerTarget(Vector3.one, true);
        UpdateDiameters(reticleSizeValue);
    }

    public void MakeReticleOnce()
    {
        SetPointerTarget(Vector3.one, true);
        UpdateDiameters();
    }
    ...
    public void RemoveReticleOnce()
    {
        SetPointerTarget(Vector3.one, false);
    }

...
public class StereoController : MonoBehaviour {
  ...
  private void CreateEye(GvrViewer.Eye eye) {
    ...
    //  ---------------------custom--------------------------
    // Camera tempCam = go.AddComponent<Camera>();
    // if (OutlineSystemCustom.Instance != null)
    // {
    //   OutlineSystemCustom.Instance.GetEyes(tempCam);
    // }
    // tempCam.enabled = false;
    
    //  PostProcessing 을 2개의 카메라에 추가한다.
    go.AddComponent<PostProcessingBehaviour>();
    //  -----------------------------------------------------
